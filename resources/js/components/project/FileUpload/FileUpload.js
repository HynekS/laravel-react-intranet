...
const FileUpload = ({ model, projectId, fileTypes = "*/*", isSingular, modalCloseCallback }) => {
  ...
  const filesToUpload = useSelector(store => store.upload.filesToUpload)
  const status = useSelector(store => store.upload.status)
  const uploadProgress = useSelector(store =>
